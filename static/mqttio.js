...
function connect() { // 브로커에 접속하는 함수 
    ...
    //document.getElementById("messages").innerHTML += '<span>접속 : ' + broker + ' 포트 ' + port + 
    '</span><br/>';
    //document.getElementById("messages").innerHTML += '<span>사용자 ID : ' + CLIENT_ID + 
    '</span><br/>';
...
function onConnect() {
    document.getElementById("messages").innerHTML += '<span>이용 시작되었습니다.' + '</span><br/>';
...
function subscribe(topic) {
    ...
    //document.getElementById("messages").innerHTML += '<span>Subscribing to: ' + topic + 
    '</span><br/>';
...
function unsubscribe(topic) {
    ...
    document.getElementById("messages").innerHTML += '<span>구독신청취소: 토픽 ' + topic +
        '</span><br/>';
...
function onMessageArrived(msg) { // 매개변수 msg는 도착한 MQTT 메시지를 담고 있는 객체 
    ...
    // 도착한 메시지 출력 
    //document.getElementById("messages").innerHTML += '<span>토픽 : ' + msg.destinationName + ' | ' 
    + msg.payloadString + '</span><br/>';
    if (msg.destinationName == 'ultrasonic') {
        addUseChartData(parseFloat(msg.payloadString));
    }
    if (msg.destinationName == "temperature") { // 온도 토픽 
        addEnvironmentChartData(parseFloat(msg.payloadString), 0);
    }
    if (msg.destinationName == 'humidity') { // 습도 토픽 
        addEnvironmentChartData(parseFloat(msg.payloadString), 1);
    }
    if (msg.destinationName == 'button') { //메시지가 button인 경우(버튼이 눌린 상황) 
        alert("스톱워치 일시 정지"); //웹 페이지에 알림창 띄운다 
        stopClock();
    }
}
